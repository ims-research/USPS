...
namespace USPS.Code.SQLite
{
    ...
    public sealed class SQLiteRoleProvider : RoleProvider
    {
            ...
            set
            {
                if (value.Length > MAX_APPLICATION_NAME_LENGTH)
                    throw new ProviderException(
                        String.Format(
                            "SQLiteRoleProvider error: applicationName must be less than or equal to {0} characters.",
        ...
        public static string MembershipApplicationName
        {
            ...
            set
            {
                if (value.Length > MAX_APPLICATION_NAME_LENGTH)
                    throw new ProviderException(
                        String.Format(
                            "SQLiteRoleProvider error: membershipApplicationName must be less than or equal to {0} characters.",
                            MAX_APPLICATION_NAME_LENGTH));

                _membershipApplicationName = value;
                _membershipApplicationId = ((_applicationName == _membershipApplicationName)
                                                ? _applicationId
                                                : GetApplicationId(_membershipApplicationName));
            }
        }
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            {
                throw new ProviderException(
                    "Connection string is empty for SQLiteRoleProvider. Check the web configuration file (web.config).");
            }
            ...
            if (config.Count > 0)
            {
                ...
                {
                    throw new ProviderException(
                        String.Concat("SQLiteRoleProvider configuration error: Unrecognized attribute: ", key));
                }
            }
        ...
        ///     If true, throw an exception if <paramref name="roleName" /> has one or more members and do not delete
        ///     <paramref
        ///         name="roleName" />
        ///     .
        /// </param>
        ...
        public override bool DeleteRole(string roleName, bool throwOnPopulatedRole)
        {
            ...
            try
            {
                ...
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "DELETE FROM " + ROLE_TB_NAME +
        ...
        public override string[] GetRolesForUser(string username)
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "SELECT r.RoleName FROM " + ROLE_TB_NAME + " r INNER JOIN " +
                                      USERS_IN_ROLES_TB_NAME
                                      + " uir ON r.RoleId = uir.RoleId INNER JOIN " + USER_TB_NAME +
                                      " u ON uir.UserId = u.UserId"
                                      +
                                      " WHERE (u.LoweredUsername = $Username) AND (u.ApplicationId = $MembershipApplicationId)";
        ...
        public override string[] GetUsersInRole(string roleName)
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "SELECT u.Username FROM " + USER_TB_NAME + " u INNER JOIN " +
                                      USERS_IN_ROLES_TB_NAME
                                      + " uir ON u.UserId = uir.UserId INNER JOIN " + ROLE_TB_NAME +
                                      " r ON uir.RoleId = r.RoleId"
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "SELECT COUNT(*) FROM " + USERS_IN_ROLES_TB_NAME + " uir INNER JOIN "
                                      + USER_TB_NAME + " u ON uir.UserId = u.UserId INNER JOIN " + ROLE_TB_NAME +
                                      " r ON uir.RoleId = r.RoleId "
                                      +
                                      " WHERE u.LoweredUsername = $Username AND u.ApplicationId = $MembershipApplicationId"
                                      + " AND r.LoweredRoleName = $RoleName AND r.ApplicationId = $ApplicationId";
            ...
            try
            {
                ...
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "DELETE FROM " + USERS_IN_ROLES_TB_NAME
                                      + " WHERE UserId = (SELECT UserId FROM " + USER_TB_NAME +
                                      " WHERE LoweredUsername = $Username AND ApplicationId = $MembershipApplicationId)"
                                      + " AND RoleId = (SELECT RoleId FROM " + ROLE_TB_NAME +
                                      " WHERE LoweredRoleName = $RoleName AND ApplicationId = $ApplicationId)";
        ...
        public override string[] FindUsersInRole(string roleName, string usernameToMatch)
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "SELECT u.Username FROM " + USERS_IN_ROLES_TB_NAME + " uir INNER JOIN " +
                                      USER_TB_NAME
                                      + " u ON uir.UserId = u.UserId INNER JOIN " + ROLE_TB_NAME +
                                      " r ON r.RoleId = uir.RoleId"
                                      +
