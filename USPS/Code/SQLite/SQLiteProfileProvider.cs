...
namespace USPS.Code.SQLite
{
    ...
    public sealed class SQLiteProfileProvider : ProfileProvider
    {
            ...
            set
            {
                if (value.Length > MAX_APPLICATION_NAME_LENGTH)
                    throw new ProviderException(
                        String.Format(
                            "SQLiteProfileProvider error: applicationName must be less than or equal to {0} characters.",
        ...
        public static string MembershipApplicationName
        {
            ...
            set
            {
                if (value.Length > MAX_APPLICATION_NAME_LENGTH)
                    throw new ProviderException(
                        String.Format(
                            "SQLiteProfileProvider error: membershipApplicationName must be less than or equal to {0} characters.",
                            MAX_APPLICATION_NAME_LENGTH));
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            {
                throw new ProviderException(
                    "Connection String is empty for SQLiteProfileProvider. Check the web configuration file (web.config).");
            }
        ...
        /// </summary>
        /// <param name="sc">
        ///     The <see cref="SettingsContext" /> that contains user profile information.
        /// </param>
        /// <param name="properties">
        ///     A <see cref="SettingsPropertyCollection" /> containing profile information for the properties to be retrieved.
        /// </param>
        /// <returns>
        ///     A <see cref="SettingsPropertyValueCollection" /> containing profile property information and values.
        /// </returns>
        ...
        /// </param>
        /// <param name="properties">
        ///     A <see cref="SettingsPropertyValueCollection" /> containing profile information and
        ///     values for the properties to be updated.
        /// </param>
        public override void SetPropertyValues(SettingsContext sc, SettingsPropertyValueCollection properties)
        {
            ...
            try
            {
                ...
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    ...
                    {
                        cmd.CommandText = "UPDATE " + PROFILE_TB_NAME +
                                          " SET PropertyNames = $PropertyNames, PropertyValuesString = $PropertyValuesString, PropertyValuesBinary = $PropertyValuesBinary, LastUpdatedDate = $LastUpdatedDate WHERE UserId = $UserId";
                    }
                    else
                    {
                        cmd.CommandText = "INSERT INTO " + PROFILE_TB_NAME +
                                          " (UserId, PropertyNames, PropertyValuesString, PropertyValuesBinary, LastUpdatedDate) VALUES ($UserId, $PropertyNames, $PropertyValuesString, $PropertyValuesBinary, $LastUpdatedDate)";
                    }
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("$UserId", userId);
                    ...
                    cmd.CommandText = "UPDATE " + USER_TB_NAME +
                                      " SET LastActivityDate = $LastActivityDate WHERE UserId = $UserId";
                    cmd.Parameters.Clear();
        ...
        ///     A <see cref="T:System.DateTime" /> that identifies which user profiles are considered inactive. If the
        ///     <see
                                                   ...
                                                   DateTime userInactiveSinceDate)
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "DELETE FROM " + PROFILE_TB_NAME + " WHERE UserId IN (SELECT UserId FROM " +
                                      USER_TB_NAME
                                      +
                                                             ...
                                                             int pageIndex, int pageSize, out int totalRecords)
        {
            string sqlQuery = "SELECT u.UserName, u.IsAnonymous, u.LastActivityDate, p.LastUpdatedDate, length(p.PropertyNames) + length(p.PropertyValuesString) FROM "
                              + USER_TB_NAME + " u, " + PROFILE_TB_NAME +
                              " p WHERE u.ApplicationId = $ApplicationId AND u.UserId = p.UserId "
                              + GetClauseForAuthenticationOptions(authenticationOption);
                                                                     ...
                                                                     int pageSize, out int totalRecords)
        {
            string sqlQuery = "SELECT u.UserName, u.IsAnonymous, u.LastActivityDate, p.LastUpdatedDate, length(p.PropertyNames) + length(p.PropertyValuesString) FROM "
                              + USER_TB_NAME + " u, " + PROFILE_TB_NAME +
                              " p WHERE u.ApplicationId = $ApplicationId AND u.UserId = p.UserId AND u.LastActivityDate <= $LastActivityDate"
                              + GetClauseForAuthenticationOptions(authenticationOption);
        ...
        /// <param name="totalRecords">When this method returns, contains the total number of profiles.</param>
        /// <returns>
        ///     A <see cref="T:System.Web.Profile.ProfileInfoCollection" /> containing user-profile information for profiles where the user name matches the supplied
        ///     <paramref
                                                                     ...
                                                                     out int totalRecords)
        {
            string sqlQuery = "SELECT u.UserName, u.IsAnonymous, u.LastActivityDate, p.LastUpdatedDate, length(p.PropertyNames) + length(p.PropertyValuesString) FROM "
                              + USER_TB_NAME + " u, " + PROFILE_TB_NAME +
                              " p WHERE u.ApplicationId = $ApplicationId AND u.UserId = p.UserId AND u.LoweredUserName LIKE $UserName"
                              + GetClauseForAuthenticationOptions(authenticationOption);
        ...
        /// <param name="totalRecords">When this method returns, contains the total number of profiles.</param>
        /// <returns>
        ///     A <see cref="T:System.Web.Profile.ProfileInfoCollection" /> containing user profile information for inactive profiles where the user name matches the supplied
        ///     <paramref
        ///         name="usernameToMatch" />
        ///     parameter.
        /// </returns>
            ...
            int pageIndex, int pageSize, out int totalRecords)
        {
            string sqlQuery = "SELECT u.UserName, u.IsAnonymous, u.LastActivityDate, p.LastUpdatedDate, length(p.PropertyNames) + length(p.PropertyValuesString) FROM "
                              + USER_TB_NAME + " u, " + PROFILE_TB_NAME +
                              " p WHERE u.ApplicationId = $ApplicationId AND u.UserId = p.UserId AND u.UserName LIKE $UserName AND u.LastActivityDate <= $LastActivityDate"
                              + GetClauseForAuthenticationOptions(authenticationOption);
        ...
        private static void CreateAnonymousUser(string username, SQLiteConnection cn, SQLiteTransaction tran,
                                                string userId)
        {
            using (SQLiteCommand cmd = cn.CreateCommand())
            {
                ...
                cmd.Parameters.Add("$PasswordFormat", DbType.String, 128).Value =
                    Membership.Provider.PasswordFormat.ToString();
        ...
        private static void GetPropertyValuesFromDatabase(string username, SettingsPropertyValueCollection svc)
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    ...
                    {
                        // User exists?
                        cmd.CommandText = "SELECT PropertyNames, PropertyValuesString, PropertyValuesBinary FROM " +
                                          PROFILE_TB_NAME + " WHERE UserId = $UserId";
                        cmd.Parameters.Clear();
                        cmd.Parameters.AddWithValue("$UserId", userId);
                ...
                try
                {
                    using (SQLiteCommand cmd = cn.CreateCommand())
                    {
                        ...
                        if ((_applicationName != _membershipApplicationName) &&
                            (String.IsNullOrEmpty(_membershipApplicationId)))
                        {
        ...
        private static ProfileInfoCollection GetProfilesForQuery(string sqlQuery, SQLiteParameter[] args, int pageIndex,
                                                                 int pageSize, out int totalRecords)
        {
            if (pageIndex < 0)
        ...
        private static object Deserialize(SettingsPropertyValue prop, object obj)
        {
            ...
            if (val == null)
            {
                ...
                {
                    val = GetObjectFromString(prop.Property.PropertyType, prop.Property.SerializeAs,
                                              (string) prop.Property.DefaultValue);
                }

                if (val != null && !prop.Property.PropertyType.IsAssignableFrom(val.GetType()))
                    // is it the correct type
                    throw new ArgumentException("Could not create from default value for property: " +
                                                prop.Property.Name);
            }
        ...
        private static string ConvertObjectToString(object propValue, Type type, SettingsSerializeAs serializeAs,
                                                    bool throwOnError)
        {
            ...
            try
            {
                switch (serializeAs)
                {
                    case SettingsSerializeAs.String:
                        ...
                        throw new ArgumentException("Unable to convert type " + type + " to string", "type");
        ...
        private static object SerializePropertyValue(SettingsPropertyValue prop)
        {
            ...
            if (prop.Property.SerializeAs != SettingsSerializeAs.Binary)
                return ConvertObjectToString(val, prop.Property.PropertyType, prop.Property.SerializeAs,
                                             prop.Property.ThrowOnErrorSerializing);
        ...
        private static object GetObjectFromString(Type type, SettingsSerializeAs serializeAs, string attValue)
        {
            ...
            switch (serializeAs)
            {
                ...
                case SettingsSerializeAs.String:
                    ...
                    throw new ArgumentException("Unable to convert type: " + type + " from string", "type");
            ...
            switch (authenticationOption)
            {
                ...
                default:
                    throw new InvalidEnumArgumentException(
                        String.Format("Unknown ProfileAuthenticationOption value: {0}.", authenticationOption.ToString()));
            }
        }
        ...
        private static SQLiteConnection GetDbConnectionForProfile()
        {
            ...
            {
                SQLiteTransaction tran = (SQLiteTransaction) HttpContext.Current.Items[HTTP_TRANSACTION_ID];
