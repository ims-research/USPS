...
namespace USPS.Code.SQLite
{
    ...
    public sealed class SQLiteMembershipProvider : MembershipProvider
    {
            ...
            set
            {
                if (value.Length > MAX_APPLICATION_NAME_LENGTH)
                    throw new ProviderException(
                        String.Format(
                            "SQLiteMembershipProvider error: applicationName must be less than or equal to {0} characters.",
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (_minRequiredNonAlphanumericCharacters > _minRequiredPasswordLength)
            {
                throw new HttpException(
                    "SQLiteMembershipProvider configuration error: minRequiredNonalphanumericCharacters can not be greater than minRequiredPasswordLength. Check the web configuration file (web.config).");
            }
            ...
            {
                throw new ProviderException(
                    "SQLiteMembershipProvider configuration error: enablePasswordRetrieval can not be set to true when passwordFormat is set to \"Hashed\". Check the web configuration file (web.config).");
            }
            ...
            if (ConnectionStringSettings == null || ConnectionStringSettings.ConnectionString == null ||
                ConnectionStringSettings.ConnectionString.Trim().Length == 0)
            {
                throw new ProviderException(
                    "Connection string is empty for SQLiteMembershipProvider. Check the web configuration file (web.config).");
            }
            ...
            if (config.Count > 0)
            {
                ...
                {
                    throw new ProviderException(
                        String.Concat("SQLiteMembershipProvider configuration error: Unrecognized attribute: ", key));
                }
            }
        ...
        public override bool ChangePassword(string username, string oldPassword, string newPassword)
        {
            ...
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                          "There must be at least {0} non alpha numeric characters.",
            ...
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                          "The password is too long: it must not exceed {0} chars after encrypting.",
                                                          MAX_PASSWORD_LENGTH));
            }
                                                             ...
                                                             string newPasswordQuestion, string newPasswordAnswer)
        {
            ...
            SecUtility.CheckParameter(ref newPasswordQuestion, RequiresQuestionAndAnswer, RequiresQuestionAndAnswer,
                                      false, MAX_PASSWORD_QUESTION_LENGTH, "newPasswordQuestion");
            if (newPasswordAnswer != null)
            {
                newPasswordAnswer = newPasswordAnswer.Trim();
            }

            SecUtility.CheckParameter(ref newPasswordAnswer, RequiresQuestionAndAnswer, RequiresQuestionAndAnswer, false,
                                      MAX_PASSWORD_ANSWER_LENGTH, "newPasswordAnswer");
            ...
            SecUtility.CheckParameter(ref encodedPasswordAnswer, RequiresQuestionAndAnswer, RequiresQuestionAndAnswer,
                                      false, MAX_PASSWORD_ANSWER_LENGTH, "newPasswordAnswer");
                                                  ...
                                                  object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            if (
                !SecUtility.ValidateParameter(ref encodedPasswordAnswer, RequiresQuestionAndAnswer, true, false,
                                              MAX_PASSWORD_ANSWER_LENGTH))
            {
                status = MembershipCreateStatus.InvalidAnswer;
                return null;
            }
            ...
            if (
                !SecUtility.ValidateParameter(ref email, RequiresUniqueEmail, RequiresUniqueEmail, false,
                                              MAX_EMAIL_LENGTH))
            {
            ...
            if (
                !SecUtility.ValidateParameter(ref passwordQuestion, RequiresQuestionAndAnswer, true, false,
                                              MAX_PASSWORD_QUESTION_LENGTH))
            {
        ...
        public override bool DeleteUser(string username, bool deleteAllRelatedData)
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    ...
                    {
                        cmd.CommandText = "SELECT UserId FROM " + USER_TB_NAME +
                                          " WHERE LoweredUsername = $Username AND ApplicationId = $ApplicationId";
                    ...
                    cmd.CommandText = "DELETE FROM " + USER_TB_NAME +
                                      " WHERE LoweredUsername = $Username AND ApplicationId = $ApplicationId";
                    ...
                    using (SQLiteDataReader dr = cmd.ExecuteReader((CommandBehavior.SingleRow)))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            ...
                            passwordFormat =
                                ...
                                Enum.Parse(typeof (MembershipPasswordFormat), dr.GetString(1));
                        ...
                        if (RequiresQuestionAndAnswer &&
                            !ComparePasswords(answer, passwordAnswer, passwordSalt, passwordFormat))
                        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "SELECT Username" +
                                      " FROM " + USER_TB_NAME +
                                      " WHERE LoweredEmail = $Email AND ApplicationId = $ApplicationId";
        ...
        public override string ResetPassword(string username, string passwordAnswer)
        {
            ...
            GetPasswordWithFormat(username, out status, out passwordFromDb, out passwordFormat, out salt,
                                  out failedPwdAttemptCount, out failedPwdAnswerAttemptCount, out isApproved,
            ...
            if (!string.IsNullOrEmpty(passwordAnswer))
            {
                encodedPwdAnswer = EncodePassword(passwordAnswer.ToLower(CultureInfo.InvariantCulture), passwordFormat,
                                                  salt);
            }
            else
            {
            ...
            SecUtility.CheckParameter(ref encodedPwdAnswer, RequiresQuestionAndAnswer, RequiresQuestionAndAnswer, false,
                                      MAX_PASSWORD_ANSWER_LENGTH, "passwordAnswer");
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    ...
                    using (SQLiteDataReader dr = cmd.ExecuteReader(CommandBehavior.SingleRow))
                    {
                        ...
                        if (RequiresQuestionAndAnswer &&
                            !ComparePasswords(passwordAnswer, passwordAnswerFromDb, salt, passwordFormat))
                        {
                            UpdateFailureCount(username, "passwordAnswer", false);
                    ...
                    {
                        throw new MembershipPasswordException(
                            "User not found, or user is locked out. Password not reset.");
                    }
                }
            }
            finally
        ...
        {
            if (!SecUtility.ValidateParameter(ref username, true, true, true, MAX_USERNAME_LENGTH) ||
                !SecUtility.ValidateParameter(ref password, true, true, false, MAX_PASSWORD_LENGTH))
            {
                return false;
            }
        ...
        private static void VerifyApplication()
        {
            ...
            try
            {
                using (SQLiteCommand cmd = cn.CreateCommand())
                {
                    cmd.CommandText = "INSERT INTO " + APP_TB_NAME +
                                      " (ApplicationId, ApplicationName, Description) VALUES ($ApplicationId, $ApplicationName, $Description)";
        ...
        private void UpdateFailureCount(string username, string failureType, bool isAuthenticated)
        {
            ...
            if (!((failureType == "password") || (failureType == "passwordAnswer")))
            {
                throw new ArgumentException(
                    "Invalid value for failureType parameter. Must be 'password' or 'passwordAnswer'.", "failureType");
            }
        ...
        private bool ComparePasswords(string password, string dbpassword, string salt,
                                      MembershipPasswordFormat passwordFormat)
        {
                                   ...
                                   out MembershipPasswordFormat passwordFormat)
        {
            ...
            DateTime lastLoginDate;
            DateTime lastActivityDate;
            GetPasswordWithFormat(username, out status, out encodedPwdFromDatabase, out passwordFormat, out salt,
                                  out failedPwdAttemptCount, out failedPwdAnswerAttemptCount, out isApproved,
                                  out lastLoginDate, out lastActivityDate);
            if (status != 0)
            {
        ...
        /// <param name="status">
        ///     The return status of the method. Possible values are: 0 = User is found and not locked;
        ///     1 = User not found; 99 = User is locked. These values match the return values of the corresponding method in
        ///     SqlMembershipProvider, so don't blame me for this goofy implementation.
        /// </param>
        /// <param name="password">
        ///     The password as stored in the database. If it is stored encrypted, the encrypted value
        ///     is returned. The calling method is responsible for decrypting it.
        /// </param>
        ...
        ///     if set to <c>true</c> the user is approved (not locked out).
        /// </param>
        /// <param name="lastLoginDate">The last login date.</param>
        /// <param name="lastActivityDate">The last activity date.</param>
        private static void GetPasswordWithFormat(string username, out int status, out string password,
            ...
            switch (passwordFormat)
            {
                ...
                case MembershipPasswordFormat.Hashed:
                    ...
                    if (algorithm == null)
                    {
                        throw new ProviderException(
                            String.Concat(
                                "SQLiteMembershipProvider configuration error: HashAlgorithm.Create() does not recognize the hash algorithm ",
        ...
        /// </summary>
        /// <returns>
        ///     A <see cref="SQLiteConnection" /> object.
        /// </returns>
        /// <remarks>
        ...
        private static SQLiteConnection GetDBConnectionForMembership()
        {
            ...
            {
                SQLiteTransaction tran = (SQLiteTransaction) HttpContext.Current.Items[_httpTransactionId];
    ...
    internal class SecUtility
    {
        ...
        /// <param name="checkForNull">
        ///     When <c>true</c>, verify <paramref name="param" /> is not null.
        /// </param>
        /// <param name="checkIfEmpty">
        ///     When <c>true</c> verify <paramref name="param" /> is not an empty string.
        /// </param>
        /// <param name="checkForCommas">
        ///     When <c>true</c> verify <paramref name="param" /> does not contain a comma.
        /// </param>
        /// <param name="maxSize">
        ///     The maximum allowed length of <paramref name="param" />.
        /// </param>
        /// <param name="paramName">Name of the parameter to check. This is passed to the exception if one is thrown.</param>
        /// <exception cref="ArgumentNullException">
        ///     Thrown when <paramref name="param" /> is null and <paramref name="checkForNull" /> is true.
        /// </exception>
        /// <exception cref="ArgumentException">
        ///     Thrown if <paramref name="param" /> does not satisfy one of the remaining requirements.
        /// </exception>
                ...
                if ((maxSize > 0) && (param.Length > maxSize))
                {
                    throw new ArgumentException(
                        String.Format("The parameter '{0}' is too long: it must not exceed {1} chars in length.",
                                      paramName, maxSize.ToString(CultureInfo.InvariantCulture)), paramName);
                }
                if (checkForCommas && param.Contains(","))
                {
                    throw new ArgumentException(
                        String.Format("The parameter '{0}' must not contain commas.", paramName), paramName);
                }
            }
        }
        ...
        /// </param>
        /// <returns>
        ///     Returns <c>true</c> if all requirements are met; otherwise returns <c>false</c>.
        /// </returns>
